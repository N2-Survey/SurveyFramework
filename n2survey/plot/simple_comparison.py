...
def form_bar_positions(df, bar_positions=False, 
                       totalbar=False, answer_supress=False):
    '''
    ...
    for entry in df:
        ...
        totalbar = False
        if all([COUNT >0, 
                len(bar_positions_complete)== BAR_POS_LENGTH-len(PERCENTAGE)]):
...
def simple_comparison_plot(df,
    ...
    bar_positions_complete = form_bar_positions(df, bar_positions,
                                                totalbar=totalbar,
    ...
    for answer,percentage in zip(all_answers, np.transpose(np.array(percentage_all))):
        ax.bar(bar_positions_complete, percentage, bottom=bottom, label=answer,
                   width=bar_width)
